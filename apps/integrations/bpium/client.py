...
class BpiumClient:
    ...
    def save_text_note(self, telegram_user_id: str, text: str) -> dict:
        ...
        self._request(
            method="POST",
            path=f"/api/catalogs/{self.notes_catalog_id}/records",
            json=payload,
        )
    ...
    def _find_user_by_telegram_id(self, telegram_user_id: str) -> dict | None:
        ...
        response_json = self._request(
            method="POST",
            path=f"/api/catalogs/{self.users_catalog_id}/records/search",
            json=search_payload,
        )
    ...
    def _get_auth_token(self) -> str:
        ...
        try:
            ...
            raise BpiumAPIError(f"Bpium API вернул невалидный JSON при авторизации: {exc}") from exc

        token = data.get("token") or data.get("access_token")
        if not token:
            raise BpiumAPIError("Bpium API не вернул токен авторизации")
